...
public class ProcessWithdrawlUpdate implements UpdateHolding {
    ...
    public Map<String,List<Holding>> updateHolding(final Map<String,List<Holding>> holdingAccounts, final TransactionModel transaction) {
        ...
        withdrawCashHolding(transaction, df, holdingList);

        return holdingAccounts;
    }

    private void withdrawCashHolding(final TransactionModel transaction, final DecimalFormat df, final List<Holding> holdingList) {
        holdingList.stream().filter(holding -> holding.getAsset().equals("CASH")).forEach( holding ->
            holding.setHoldings(
                Double.valueOf(
                    df.format(holding.getHolding() - transaction.getUnits())
                )
            )
        );
    }
